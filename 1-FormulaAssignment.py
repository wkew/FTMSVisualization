...
try: #test if running in ipython
    ...
    path  = os.getcwd()+"\\MassToFormula\\data\\" #example data location
    dictionarypath = os.getcwd()+"\\MassToFormula\\dictionaries\\"
...
minKMDseries = 3 #minimum number of peaks in a single homologous series to assign by zstar approach # 3 seems a good number. 
		...
		for x in dicta.itertuples():
			if low <= x[1] <=high:
				...
				if abs(error) <= threshppm:
					...
					formulatemp = FTPM.formulator(int(x[3]),int(x[4]),int(x[6]),int(x[5]),int(x[7]),int(x[8]),0,ionisationmode)
...
def assigningpart(zs):
    ...
    outheaders = ["Theor. Mass","Isotopic Abundance","Cno","Hno","Ono","Nno","Sno","Nano","Error","Rel. Abundance","Exp. m/z","DBE","Formula"]
    assignedDF = pd.DataFrame(assigned,columns=outheaders)
    newcols = ["Exp. m/z","Theor. Mass","Error","Rel. Abundance","DBE","Cno","Hno","Nno","Ono","Sno","Formula"]#,"Nano","Isotopic Abundance"]
    if ionisationmode =="negative":
        assignedDF["Hno"] = assignedDF["Hno"].add(1)
...
def isotopechecker(unassignedDF,assignedDF):    
    ...
    isotopeheaders = ["Exp. m/z","Recal m/z","Theor. Mass","Error","Rel. Abundance","Signal2Noise","DBE","Cno","Hno","Nno","Ono","Sno","13Cno","18Ono","Formula","HeteroClass"]
    a= 0
    newlist = []
